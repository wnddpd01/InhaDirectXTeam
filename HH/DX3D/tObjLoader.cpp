...
void tObjLoader::Load(std::vector<tGroup*>& vecGroup, char* szFolder, char* szFile)
{
	...
	while (true)
	{
		...
		else if(szTemp[0] == 'g')
		{
			if(!vecVertex.empty())
			{
				...
				{
					D3DXVec3TransformNormal(&vecVertex[i].p, &vecVertex[i].p, &matWorld);
				}


				
				tGroup* pGroup = new tGroup;
...
void tObjLoader::LoadMtlLib(char* szFolder, char* szFile)
{
	std::string sFullPath(szFolder);
	sFullPath += (std::string("/") + std::string(szFile));
	...
	while(true)
	{
		...
		else if(szTemp[0] == 'n')
		{
			...
			if(m_mapMtlTex.find(sMtlName) == m_mapMtlTex.end())
				m_mapMtlTex[sMtlName] = new tMtlTex;
		}
		else if (szTemp[0] == 'K')
		{
			if (szTemp[1] == 'a')
			{
				...
				m_mapMtlTex[sMtlName]->GetMaterial().Ambient.a = 1.f;
			}
			else if (szTemp[1] == 'd')
			{
				float r, g, b;
				...
				m_mapMtlTex[sMtlName]->GetMaterial().Diffuse.a = 1.f;
			}
			else if (szTemp[1] == 's')
			{
				float r, g, b;
				...
				m_mapMtlTex[sMtlName]->GetMaterial().Specular.a = 1.f;
			}
		}
		else if (szTemp[0] == 'd')
		{
